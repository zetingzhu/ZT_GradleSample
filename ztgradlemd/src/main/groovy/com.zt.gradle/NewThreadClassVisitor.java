...
public class NewThreadClassVisitor extends ClassNode {
    private static final String TAG = "ASM-NewThread";
    ...
    private static final String HANDLER_CLASS_NAME = "com/zzt/gradlesample/asm/AsmHandler";
    ...
    public void visitEnd() {
        System.out.println(TAG + ">>>>>>  NewThreadClassVisitor 1");
        methods.forEach(new Consumer<MethodNode>() {
            @Override
            public void accept(MethodNode methodNode) {
                System.out.println(TAG + ">>>>>>  NewThreadClassVisitor 2");
    ...
    private void handleMethodInsn(MethodNode methodNode) {
        System.out.println(TAG + ">>>>>>  NewThreadClassVisitor 3");
        InsnList insnList = methodNode.instructions;
        ListIterator<AbstractInsnNode> iterator = insnList.iterator();
        while (iterator.hasNext()) {
            AbstractInsnNode insnNode = iterator.next();
            System.out.println(TAG + ">>>>>>  NewThreadClassVisitor 4:" + insnNode);
            if (insnNode instanceof MethodInsnNode) {
                MethodInsnNode methodInsn = (MethodInsnNode) insnNode;
                if (checkIsTargetInvoke(methodInsn)) {

                    System.out.println(TAG + ">>>>>>  NewThreadClassVisitor 6   handleMethod class: " + name + ",method=" + methodNode.name + ", desc=" + methodNode.desc);
...
//                    printInsnList(methodNode);
                    ...
                    MethodInsnNode newMethodInsn = new MethodInsnNode(Opcodes.INVOKESTATIC, HANDLER_CLASS_NAME, HANDLER_METHOD_NAME, HANDLER_METHOD_DESC);
    ...
    private boolean checkIsTargetInvoke(MethodInsnNode methodInsn) {
        ...
        boolean isTarget = (opcode == Opcodes.INVOKESPECIAL && THRED_CLASS_NAME.equals(methodInsn.owner) && CONSTRUCTOR_METHOD.equals(methodInsn.name));

        System.out.println(TAG + ">>>>>>  NewThreadClassVisitor 5:" + isTarget);
        if (isTarget) {
